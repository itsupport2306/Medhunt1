...
const SOURCING_PLATFORMS = {
  ...
  npino: {
    ...
    adapterRevision: "healthcare-directory-v3",
    adapterRequestType: "RADIXSOL_HEALTHCARE_DIRECTORY_V3_REQUEST",
    resumeCapture: false,
  },
  nysed: {
    key: "nysed",
...
function viewAdd() {
  ...
  $("#content").innerHTML = `
    ${IS_EXTENSION ? `<div class="card capture-card">
      ...
      <p class="muted small">Open an authorized candidate or provider page in Indeed, Vivian, ZipRecruiter, LinkedIn, Facebook, NPI No., NPI Profile, U.S. News Doctor Finder, or NYSED, then capture it for review and contact enrichment.</p>
...
async function loadAuth() {
  ...
  try {
    ...
    authConfig = { enabled: false, provider: "healthboard" };
  }
  renderAuthState();
}
...
async function login() {
  ...
  $("#modalRoot").innerHTML = `<div class="modal" role="presentation">
    <div class="dialog" role="dialog" aria-modal="true" aria-labelledby="medhuntLoginTitle">
      <h2 id="medhuntLoginTitle">Sign in to Medhunt</h2>
...
async function requestLoginCode() {
  ...
  pendingLogin = { email, challenge: response.challenge };
  $("#modalRoot").innerHTML = `<div class="modal" role="presentation">
    <div class="dialog" role="dialog" aria-modal="true" aria-labelledby="medhuntCodeTitle">
      <h2 id="medhuntCodeTitle">Enter your email code</h2>
...
function sameUsNewsProfileUrl(firstValue, secondValue) {
  try {
    const first = new URL(firstValue);
    const second = new URL(secondValue);
...
async function captureUsNewsProfileInBackground(profile, timeoutMs = 45000) {
  const sourceUrl = String(profile?.source_url || "").trim();
  if (!sourceUrl || platformForUrl(sourceUrl)?.key !== "usnews") {
    throw new Error("The U.S. News profile link is unavailable.");
  }
  const tab = await chrome.tabs.create({ url: sourceUrl, active: false });
  const tabId = Number(tab?.id);
  if (!tabId) throw new Error("The U.S. News profile tab could not be opened.");
  const deadline = Date.now() + timeoutMs;
  try {
    let loaded = false;
    while (Date.now() < deadline) {
      ...
      if (!current) throw new Error("The U.S. News profile tab was closed.");
      if (sameUsNewsProfileUrl(current.url || "", sourceUrl) && current.status === "complete") {
        loaded = true;
        break;
      }
      await wait(250);
    }
    if (!loaded) throw new Error("U.S. News did not finish opening the exact candidate profile.");

    const message = {
      type: SOURCING_PLATFORMS.usnews.adapterRequestType,
    ...
    while (Date.now() < deadline) {
      try {
        ...
        lastError = new Error(response?.error || "The U.S. News profile is still loading its credentials.");
      } catch (error) {
        lastError = error;
    ...
    throw lastError || new Error("U.S. News did not expose the professional profile details.");
  } finally {
    await chrome.tabs.remove(tabId).catch(() => {});
  }
}

function usNewsImportPayload(profile) {
...
async function enrichUsNewsProfileAndResume(profile) {
  ...
  const captured = await captureUsNewsProfileInBackground(profile);
  if (!captured?.profile_document) throw new Error("The U.S. News profile did not contain professional details.");
  ...
  await api("/candidates/import/batch", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ profiles: [usNewsImportPayload(profile)], job_id: activeJobId, search_url: activeSourcingPageUrl || "" }),
    timeout: 120000,
  });
  return ensureProfessionalProfileResume(profile);
}

function startUsNewsResumeBatch(profiles) {
  const queue = (profiles || []).filter((profile) => (
    profile?.source === "usnews" && Number(profile._candidateId) && !indeedLookupFor(profile).resume
  ));
  ...
  return (async () => {
    try {
      for (const profile of queue) {
        let saved = false;
        try {
          saved = Boolean(await enrichUsNewsProfileAndResume(profile));
        } catch (error) {
      ...
      if (completed.saved) {
        notify(
          `${completed.saved} U.S. News profile resume${completed.saved === 1 ? "" : "s"} saved` +
          `${completed.failed ? `; ${completed.failed} unavailable` : ""}.`,
          completed.failed ? "error" : "",
        );
      } else if (completed.failed) {
        notify("U.S. News profile resumes could not be saved.", "error");
      }
    }
  })();
}
...
function filterDisplayedCandidateRows(value = indeedCandidateQuery) {
  indeedCandidateQuery = String(value || "").trim().toLowerCase();
  let visible = 0;
  document.querySelectorAll(".capture-row[data-search]").forEach((row) => {
    const matches = !indeedCandidateQuery || row.dataset.search.includes(indeedCandidateQuery);
    row.hidden = !matches;
    if (matches) visible += 1;
  });
  const empty = $("#candidateFilterEmpty");
  if (empty) empty.hidden = visible > 0;
  const clear = $("#clearCandidateFilter");
  if (clear) clear.hidden = !indeedCandidateQuery;
}
...
function indeedFilteredProfiles() {
  const inLookupView = ["lookup", "results"].includes(indeedScanState.phase);
...
function renderIndeedProfiles(scan = {}) {
  ...
  $("#content").innerHTML = `
    <section class="indeed-workflow">
      ...
      ${profiles.length > 5 || indeedCandidateQuery ? `
        <div class="candidate-search-bar">
          <label for="candidateSearch"><span aria-hidden="true"></span><span class="sr-only">Filter displayed profiles</span></label>
          <input id="candidateSearch" type="search" autocomplete="off" placeholder="Filter by name, location, or role" value="${escapeHtml(indeedCandidateQuery)}">
          <button type="button" id="clearCandidateFilter" data-action="clear-candidate-filter" aria-label="Clear profile filter"${indeedCandidateQuery ? "" : " hidden"}>&times;</button>
        </div>` : ""}

      <div class="indeed-candidate-list${hasResults || isLookingUp ? " result-list" : ""}" id="indeedCandidateList" data-testid="profile-list">
        ${profiles.length
          ...
          return `<article class="capture-row${indeedSelected.has(key) ? " selected" : ""}" data-profile-key="${escapeHtml(key)}" data-search="${escapeHtml(searchText)}">
            ${hasResults || isLookingUp ? "" : `<label class="candidate-select-control"><input type="checkbox" class="indeed-select" data-key="${escapeHtml(key)}"${indeedSelected.has(key) ? " checked" : ""}><span class="sr-only">Select ${escapeHtml(profile.name)}</span></label>`}
            <div class="capture-avatar" aria-hidden="true">${escapeHtml(initials(profile.name))}</div>
            <button type="button" class="capture-identity" data-action="open-indeed-result" data-index="${originalIndex}" aria-disabled="${indeedResumeBatchState.active ? "true" : "false"}"${indeedResumeBatchState.active ? " disabled" : ""}>
        ...
        <div id="candidateFilterEmpty" class="empty-results compact" hidden><strong>No matching profiles</strong><span>Try another name, location, or role.</span></div>
      </div>
...
async function ensureProfessionalProfileResume(profile) {
  ...
  if (
    profile?.source !== "usnews" || !candidateId
...
async function performDisplayedIndeedSave(searchUrl) {
  ...
  try {
    ...
    (result.results || []).forEach((saved, index) => {
      ...
      if ((currentProfile || source).profile_document) {
        professionalProfiles.push(currentProfile || source);
      }
    });
  ...
  if (IS_EXTENSION) {
    try {
      ...
      renderSourcingStatus(
        ...
        "Open Indeed, Vivian, ZipRecruiter, LinkedIn, Facebook, NPI No., NPI Profile, U.S. News Doctor Finder, or NYSED. The panel will detect it automatically.",
        { retry: false },
      );
      return;
    }
...
async function synchronizeActiveSourcingTab(reason = "changed") {
  ...
  if (!tab?.id || !platform) {
    ...
    renderSourcingStatus(
      "Ready for a candidate page",
      "Open Indeed, Vivian, ZipRecruiter, LinkedIn, Facebook, NPI No., NPI Profile, U.S. News Doctor Finder, or NYSED. The panel will update automatically.",
      { retry: false },
    );
    return;
  }
...
async function lookupSelectedIndeedCandidates() {
  ...
  function applyLookupResult(profile, lookup) {
    ...
    if (profile.source === "usnews" && !result.resume) {
      usNewsResumeQueue.push(profile);
    }
  ...
  notify(
    ...
    if (usNewsResumeQueue.length) {
      await startUsNewsResumeBatch(usNewsResumeQueue);
...
document.addEventListener("input", (event) => {
  if (event.target.id !== "candidateSearch") return;
  filterDisplayedCandidateRows(event.target.value);
});

document.addEventListener("keydown", (event) => {
  if (event.target.id === "candidateSearch" && event.key === "Escape") {
