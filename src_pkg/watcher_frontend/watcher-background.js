...
// `running` is persisted so the panel can show progress, but the actual lock
// is the in-memory activeCycle. If Chrome stops the service worker while a
// fetch is in flight, the persisted flag survives without the promise that
// would have cleared it. Recover that abandoned run before the next check so
// alarms and the Check now control are not blocked by stale state.
async function recoverAbandonedRun() {
  if (activeCycle) return false;
  const stored = await chrome.storage.local.get([STATE_KEY]);
  const saved = stored[STATE_KEY];
  if (!saved?.running) return false;
  const current = { ...defaultState(), ...saved };
  const detail = current.lastError || "The previous check was interrupted. Ready to retry.";
...
function normalizedResumeMarker(marker) {
...
function classifyProfile(profile, checkpoint, observedAt) {
  ...
  // Relative labels such as "2 hours ago" are converted using the current
  // clock. Without this guard, the estimate moves forward on every scan and
  // eventually makes an unchanged card look updated.
  if (marker && normalizedResumeMarker(marker) === normalizedResumeMarker(checkpoint.resumeMarker)) {
...
async function performCycle({ useCurrentPage = false } = {}) {
  ...
  try {
    ...
    const workItems = profiles.map((profile) => {
    ...
    const changed = workItems.filter((item) => ["new", "updated"].includes(item.classification.action));
    const retries = workItems.filter((item) => item.classification.action === "retry");
    const workDescription = [
      changed.length ? `${changed.length} new or updated profile${changed.length === 1 ? "" : "s"}` : "",
      retries.length ? `${retries.length} retr${retries.length === 1 ? "y" : "ies"}` : "",
    ...
    await writeState({
      ...
      foundCount: 0,
      progressDone: 0,
      progressTotal: Math.max(1, workItems.length),
      detail: workItems.length
        ? `Processing ${workDescription}…`
        : "No new or updated profiles were detected.",
    });

    await queueResumeNotification(resumeNotificationEvent(state, workItems, observedAt));
    ...
    const imported = await api("/candidates/import/batch", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ profiles: workItems.map((item) => importProfile(item.profile)), search_url: scan.page_url || state.searchUrl }),
    });
    const saved = Array.isArray(imported.results) ? imported.results : [];
    if (saved.length !== workItems.length) throw new Error("The backend returned an incomplete candidate import.");
    saved.forEach((result, index) => { workItems[index].candidateId = Number(result.id); });

    let found = 0;
    let failed = 0;
    let resumes = 0;
    const lookupRunId = runId();
    ...
    for (let index = 0; index < workItems.length; index += 1) {
      ...
      let lookupError = "";
      try {
        // Quick Sourcer drives a live browser and needs 30-90 seconds per
        // person. A 50-person request therefore outlives the extension fetch
        // timeout while the backend keeps working. Submit one candidate at a
        // time so every completed result is checkpointed before the next one.
        ...
        result = lookup?.results?.[String(item.candidateId)] || result;
      } catch (error) {
      ...
      await writeState({
        ...
        detail: lookupError
          ? `Processed ${index + 1} of ${workItems.length}; this profile will retry.`
          : `Processed ${index + 1} of ${workItems.length}.`,
      });
    }
    ...
    const emailSummary = emailOutcome.status === "sent"
      ...
      : emailOutcome.status === "pending"
        ...
        : "";
    const summary = `${changed.length} new/updated; ${retries.length} retried; ${found} contacts; ${resumes} resumes` +
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message?.type === "MEDHUNT_WATCHER_GET_STATE") {
    recoverAbandonedRun()
      .then(() => Promise.all([readState(), backendReady()]))
